...
def _butterfly_data(results, threshold, n_bins):
    """
    Split results into bins and count below/above threshold dice scores.
    """
...
def plot_butterfly_mask_vs_score(
    n_bins,
    threshold,
    pre_results,
    post_results,
    ...
    """
    Generate butterfly plot comparing pre-oversample vs post-oversample performance.
    """
    ...
    # LEFT: PRE
    ax = axes[0]
    ax.bar(index, -np.array(pre_below), bar_width, color="blue", label=f"< Dice {threshold}")
    ax.bar(index, np.array(pre_above), bar_width, color="red", label=f">= Dice {threshold}")
    ...
    # RIGHT: POST
    ax = axes[1]
    ax.bar(index, -np.array(post_below), bar_width, color="blue", label=f"< Dice {threshold}")
    ax.bar(index, np.array(post_above), bar_width, color="red", label=f">= Dice {threshold}")
    ax.axhline(y=0, color="black", linestyle="--")
